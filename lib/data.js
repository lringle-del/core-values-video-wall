...
const { getSheets, isConfigured, SHEET_TAB, SHEET_HEADER, COMMENTS_TAB, COMMENTS_HEADER } = require('./google');
const { DEMO_ENTRIES } = require('./demo-wall');

const devRows = [];
const devComments = [];

// Creates the tab (with header) if it doesn't exist yet, so the user only
// has to set up the "Submissions" tab by hand — this one's automatic.
async function ensureTab(sheets, tabName, header) {
  const spreadsheetId = process.env.SHEET_ID;
  try {
    const res = await sheets.spreadsheets.values.get({ spreadsheetId, range: `${tabName}!A1:Z1` });
    if (!res.data.values || res.data.values.length === 0) {
      await sheets.spreadsheets.values.update({
        spreadsheetId, range: `${tabName}!A1`, valueInputOption: 'RAW', requestBody: { values: [header] },
      });
    }
  } catch (err) {
    // Tab doesn't exist yet — create it, then write the header.
    await sheets.spreadsheets.batchUpdate({
      spreadsheetId, requestBody: { requests: [{ addSheet: { properties: { title: tabName } } }] },
    });
    await sheets.spreadsheets.values.update({
      spreadsheetId, range: `${tabName}!A1`, valueInputOption: 'RAW', requestBody: { values: [header] },
    });
...
async function appendComment(name, comment) {
  const row = [new Date().toISOString(), name, comment];
  if (!isConfigured()) {
    devComments.unshift(row);
    return;
  }
  const sheets = await getSheets();
  await ensureTab(sheets, COMMENTS_TAB, COMMENTS_HEADER);
  await sheets.spreadsheets.values.append({
    spreadsheetId: process.env.SHEET_ID,
...
async function getComments() {
  if (!isConfigured()) {
    return devComments.length ? devComments.map(rowToComment) : DEMO_COMMENTS;
  }
  const sheets = await getSheets();
  await ensureTab(sheets, COMMENTS_TAB, COMMENTS_HEADER);
  const res = await sheets.spreadsheets.values.get({ spreadsheetId: process.env.SHEET_ID, range: `${COMMENTS_TAB}!A2:C` });
  const rows = res.data.values || [];
  return rows.map(rowToComment).reverse(); // newest first
}
...
const DEMO_COMMENTS = [
  { name: 'Laura Jackson', comment: 'So fun to see everyone\'s videos! Love this idea.' },
  { name: 'Jake Adkisson', comment: 'Great way to show what makes this place special.' },
];

module.exports = { getParticipantEmails, getUniqueParticipantCount, appendSubmission, getWallEntries, appendComment, getComments };
